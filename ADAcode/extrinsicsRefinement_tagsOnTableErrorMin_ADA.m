...
addpath(strcat(pwd, '/dataFromADA'))
cd(d)
...
for i=1:size(tagsPose,1) 
    ...
    % for ADA
    t_w(:, :, i) = T_tb2rb * T_rb2e * K_wc * t_c;
    zhat_tag(i, :) = t_w(1:3, 3);
end
...
[x, fval, exittag] = fmincon(@minimize_tableTagErrorADA,x0,[],[],[],[],[],[],[], options, tagsPose, x0);
